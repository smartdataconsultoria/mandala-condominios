...
function hdrs(){
  const tk = getToken();
  if(!tk){
    try{ showAlert('Sessão expirada. Faça login novamente.','error'); }catch(e){}
    throw new Error('Sem access_token no navegador');
  }
  return {
...
function renderCards(){
    ...
    return `<div class="morador-card" onclick="abrirModal(${m.id})">
      ...
      <div class="card-bottom">
        ...
        <div class="card-actions" onclick="event.stopPropagation()">
          <button class=\"btn-icon btn-edit\" data-id=\"${m.id}\" title=\"Editar\">✏</button>
  ...
  // Liga eventos dos botões (evita JS inválido no onclick com ${})
